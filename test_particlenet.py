...
plt.axvline(best_loss_epoch, color='g', linestyle='--', label='best loss')
plt.legend()
#wandb.log({'loss':fig_loss})
plt.savefig(f'{save_dir}/loss.jpg')

plt.close()


# Save and log AUC plot
fig_auc = plt.figure()
plt.plot(auc_train_list, label='train')
plt.plot(auc_val_list, label='val')
plt.axvline(best_auc_epoch, color='r', linestyle='--', label='best auc')
plt.axvline(best_loss_epoch, color='g', linestyle='--', label='best loss')
plt.legend()
#wandb.log({'auc':fig_auc})
plt.savefig(f'{save_dir}/auc.jpg')
