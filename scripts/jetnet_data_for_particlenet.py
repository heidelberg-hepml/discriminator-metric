...
# convert data to pnet input coordinates

      particle_data[s] = create_pnet_inputs(particle_data[s],jet_data[s])
...
for s in new_split:
      new_jet_data[s]['data'].to_hdf('data/converted/jetnet_converted_data.h5', key=f'jet_data_{s}', mode='a',format='table')  
      new_particle_data[s]['data'].to_hdf('data/converted/jetnet_converted_data.h5', key=f'particle_data_{s}', mode='a',format='table')
      new_jet_data[s]['labels'].to_hdf('data/converted/jetnet_converted_data.h5', key=f'labels_{s}', mode='a',format = 'table')
