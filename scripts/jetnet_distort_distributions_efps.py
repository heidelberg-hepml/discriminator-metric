...
parser = argparse.ArgumentParser(description='distort jets for lorentzNet Training')
...
efps_true = np.load("data/raghav/efps_true.npy")
np.random.seed(4)
...
for key, (weights, _) in dists.items():
   #print(key)
    ...
    efp_distorted = efps_true[inds]
    np.save('data/distorted_efps/efp_'+key+'.npy', efp_distorted)
